...
class ObstacleField(object):
    ...
    def __init__(self, dt, obs_num=50):
        self.dt = dt
    ...
    def random_init_obstacle(self, t, vehicle_x = 0, vehicle_y = -1, min_dist = 0.2):
        ...
        b_x = random.uniform(5e-3, 2e-2) * random.choice([1,-1]) 
        b_y = random.uniform(5e-3, 2e-2) * random.choice([1,-1])
        a_x = 0 #random.uniform(5e-6, 1e-4) * random.choice([1,-1]) 
        a_y = 0 #random.uniform(5e-6, 1e-4) * random.choice([1,-1]) 
