...
class ObstacleField(object):
    ...
    def random_init_obstacle(self, t, vehicle_x = 0, vehicle_y = -1, min_dist = 0.2):
        dist = -1
        x = y = -1
        while (dist < min_dist):
        ...
        b_x = random.uniform(5e-3, 2e-2) * random.choice([1,-1]) #* 0.01 random.uniform(5e-3, 3e-2)
        b_y = random.uniform(5e-3, 2e-2) * random.choice([1,-1]) #* 0.01 random.uniform(5e-3, 3e-2)
        a_x = 0 #random.uniform(5e-6, 1e-4) * random.choice([1,-1]) #* 0.1
        a_y = 0 #random.uniform(5e-6, 1e-4) * random.choice([1,-1]) #* 0.1
    ...
    def random_init_obs_index(self, index, vehicle_x, vehicle_y):
        self.obstacles[index] = self.random_init_obstacle(0, vehicle_x, vehicle_y, min_dist = 0.2)
